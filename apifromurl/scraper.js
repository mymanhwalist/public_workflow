...
const WALL_LIMIT_MS          = 90 * 60 * 1000; // stop after 90 min to stay under GitHub's 2h job limit
...
// Only scrape providers that do NOT return a reliable posting date (baseline/ID collection)
const NO_DATE_PROVIDERS_ONLY = process.argv.includes('--no-date-providers');
const RELIABLE_DATE_PROVIDERS = ['Greenhouse', 'Lever', 'Ashby', 'Workable', 'Eightfold', 'Recruitee', 'BambooHR', 'Personio', 'SmartRecruiters', 'Breezy HR'];
...
async function main() {
  ...
  if (NO_DATE_PROVIDERS_ONLY) console.log(`NO-DATE PROVIDERS ONLY — excludes: ${RELIABLE_DATE_PROVIDERS.join(', ')}`);
  console.log('══════════════════════════════════════════\n');
  ...
  if (NO_DATE_PROVIDERS_ONLY) q = q.not('ats_provider', 'in', `(${RELIABLE_DATE_PROVIDERS.map(p => `"${p}"`).join(',')})`);
...
function normalizeJob(raw, config) {
  ...
  switch (ats) {
    ...
    case 'Workable': {
      const wSlug = config.api_endpoint?.match(/accounts\/([^/]+)\/jobs/)?.[1] || '';
      title           = raw.title;
      application_url = wSlug && raw.shortcode ? `https://apply.workable.com/${wSlug}/j/${raw.shortcode}` : null;
      location_raw    = raw.location ? [raw.location.city, raw.location.country].filter(Boolean).join(', ') : null;
      external_id     = raw.shortcode || String(raw.id);
      posted_date     = raw.published || raw.created_at || null;
      raw_data        = raw;
      break;
    }
...
async function upsertRawJob(job, config, isFirstScrape, skipIfNew = false) {
  try {
    ...
    // New job — determine status based on posted_date availability and first-scrape flag
    ...
    let status;
    if (!job.posted_date && !isFirstScrape) {
      status = 'skipped_no_date'; // new job ID seen but no date — collect as baseline only, refiner won't touch
    } else {
      status = (isFirstScrape && !isRecentlyPosted) ? 'skipped_first_scrape' : 'pending';
    }
    const isSkipped = status !== 'pending';
    const { error } = await db2.from('raw_jobs').insert({
      company_id:      config.company_id,
      config_id:       config.id,
      title:           job.title,
      description:     isSkipped ? null : (job.description || null),
      location_raw:    job.location_raw || null,
      application_url: job.application_url,
      external_id:     job.external_id || null,
      posted_date:     job.posted_date || null,
      raw_data:        isSkipped ? null : (job.raw_data || null),
      is_first_scrape: isFirstScrape,
      status,
    });
